...
static int my_FindDeviceProc( uint16 source, uint16 command, uint16 len, uint8 *pData) 
{
  ...
  if (command == 0xff01) {//根据MAC地址查询短地址
    uint8 *pExtAddr = dat;
    ZMacGetReq( ZMacExtAddr, pExtAddr );//获取MAC地址
  ...
  if (command == 0xff02) {//根据短地址查询MAC地址
