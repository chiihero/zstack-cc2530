/***************************************************************************************************
  Filename:       MT_SYS.c
  Revised:        $Date: 2010-10-05 10:18:33 -0700 (Tue, 05 Oct 2010) $
  Revision:       $Revision: 23986 $
...
void MT_SysOsalNVRead(uint8 *pBuf)
{
      ...
      (ZCD_NV_DEVICE_PRIVATE_KEY == nvId))
  {
    ...
    MT_BuildAndSendZToolResponse(((uint8)MT_RPC_CMD_SRSP | (uint8)MT_RPC_SYS_SYS), cmdId, 2, tmp);
    return;
  }
#endif
       ...
       (nvId == ZCD_NV_PRECFGKEY) )
  {
    ...
    MT_BuildAndSendZToolResponse(((uint8)MT_RPC_CMD_SRSP | (uint8)MT_RPC_SYS_SYS), cmdId, 2, tmp1);
    return;
  }
