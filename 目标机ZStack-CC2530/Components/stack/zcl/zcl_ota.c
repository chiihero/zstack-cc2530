/******************************************************************************
  Filename:       zcl_ota.c
  Revised:        $Date: 2010-11-18 19:04:44 -0800 (Thu, 18 Nov 2010) $
  Revision:       $Revision: 24463 $
...
uint16 zclOTA_event_loop( uint8 task_id, uint16 events )
{
  ...
  {
    // Notify the application task of the timeout waitinf for the download to start
...
static ZStatus_t zclOTA_ProcessUpgradeEndRsp( zclIncoming_t *pInMsg )
{
  ...
  // verify in 'download complete' state
  if (zclOTA_ImageUpgradeStatus != OTA_STATUS_COMPLETE)
  {
    return ZSuccess;
  }
