/**************************************************************************************************
  Filename:       zcl.c
  Revised:        $Date: 2010-11-09 15:38:54 -0800 (Tue, 09 Nov 2010) $
  Revision:       $Revision: 24374 $
...
static uint8 zclWriteAttrDataUsingCB( uint8 endpoint, zclAttrRec_t *pAttr, uint8 *pAttrData );
...
static uint8 zclReadAttrDataUsingCB( uint8 endpoint, uint16 clusterId, uint16 attrId,
 ...
 *              attribute data.
 *
 *              Note: The callback function is only required when the attribute
 *                    data format is unknown to ZCL. The callback function gets
 *                    called when the pointer 'dataPtr' to the attribute value
 *                    is NULL in the attribute database registered with the ZCL.
 *
...
static uint8 zclWriteAttrDataUsingCB( uint8 endpoint, zclAttrRec_t *pAttr, uint8 *pAttrData )
{
  if ( zcl_AccessCtrlWrite( pAttr->attr.accessControl ) )
  {
    ...
    return ( (*pfnReadWriteCB)( pAttr->clusterID, pAttr->attr.attrId, ZCL_OPER_WRITE, pAttrData, NULL ) );
  }
  ...
  for (i = 0; i < writeCmd->numAttr; i++)
  {
                         ...
                         statusRec->attrID, &attrRec ) )
    {
      if ( statusRec->dataType == attrRec.attr.dataType )
      {
        ...
        {
          status = zclWriteAttrDataUsingCB( pInMsg->msg->endPoint, &attrRec, 
                                            statusRec->attrData );
        }
                                    ...
                                    zclWriteRec_t *curWriteRec, uint16 numAttr )
{
  ...
  for (i = 0; i < numAttr; i++)
  {
    ...
    else // Use CB
    {
      ...
      zclWriteAttrDataUsingCB( pInMsg->msg->endPoint, &attrRec, statusRec->attrData );
    }
  } // for loop
}
  ...
  if ( writeRspCmd->numAttr == 0 ) // All attributes can be written
  {
    ...
    for (i = 0; i < writeCmd->numAttr; i++)
    {
      ...
      else // Use CBs
      {
        ...
        // Write the new attribute value
        status = zclWriteAttrDataUsingCB( pInMsg->msg->endPoint, &attrRec, statusRec->attrData );
      }
