...
void SAPI_ReceiveDataIndication( uint16 source, uint16 command, uint16 len, uint8 *pData  )
{
...
#endif  //MT_SAPI_CB_FUNC
  {
    ...
    _zb_ReceiveDataIndication( source, command, len, pData  );
#endif
  }
}
/*********************************************************************
...
void SAPI_ProcessZDOMsgs( zdoIncomingMsg_t *inMsg )
{
  switch ( inMsg->clusterID )
  {
...
#else
        SAPI_FindDeviceConfirm( ZB_IEEE_SEARCH, pNwkAddrRsp->extAddr,(uint8*)&pNwkAddrRsp->nwkAddr );
#endif
      }
      break;
    ...
    case IEEE_addr_rsp:
      {
        ZDO_NwkIEEEAddrResp_t *pNwkAddrRsp = ZDO_ParseAddrRsp( inMsg );
        SAPI_FindDeviceConfirm( ZB_NWKA_SEARCH, (uint8*)&pNwkAddrRsp->nwkAddr,(uint8*)&pNwkAddrRsp->extAddr );
      }
      break;
